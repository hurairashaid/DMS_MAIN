...
const ApprovedEvacuation = () => {
  const [token, setToken] = useState('');
  const [evacuations, setEvacuations] = useState([]);
  console.log(token);
  console.log(`http://${BASE_URL}/evacuation/allapprovedevacuation`);
  useEffect(() => {
  ...
  const fetchEvacuations = async token => {
    try {
      ...
      const response = await axios.post(
        `http://${BASE_URL}/evacuation/allapprovedevacuation`,
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.heading}>Approved Evacuations</Text>
      <FlatList
        data={evacuations}
        ...
        renderItem={({item}) => (
          <View style={styles.evacuationBlock}>
            ...
            <FlatList
              data={item.evacuation.filter(evac => evac.status === 'approved')}
...
export default ApprovedEvacuation;
